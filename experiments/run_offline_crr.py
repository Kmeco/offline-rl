...
flags.DEFINE_integer('seed', 0, 'Random seed for replicable results.')
...
def _build_custom_loggers():
    tag = str(int(time.time())) + FLAGS.logs_tag
    ...
    tb_logger = TFSummaryLogger(logdir=logs_dir, label='EvalLoop')
...
def main(_):
    ...
    wandb.init(project="offline-rl", sync_tensorboard=True)
